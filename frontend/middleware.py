...
class FastAPIDjangoMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: StarletteRequest, call_next):
        if [url for url in self.django_patterns if url.pattern.match(request.url.path.lstrip('/'))]:
        ...
        if response is None:
            ...
            django_response = await sync_to_async(self.django_middleware_runner.process_response)(django_request,
                                                                                                  django_response)
    ...
    async def convert_to_django_request(self, request: StarletteRequest) -> HttpRequest:
        ...
        django_request.META = {
            ...
            'HTTP_X_CSRFTOKEN': request.headers.get('x-csrftoken', request.cookies.get('csrftoken', '')),
    ...
    async def convert_to_django_response(self, response: StarletteResponse) -> HttpResponse:
    ...
    def convert_to_starlette_response(self, django_response: HttpResponse) -> StarletteResponse:
